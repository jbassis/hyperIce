...
H0 = 1e3
Lx = L0 = 40e3  # length of ice shelf
accum =0.0/mat.seconds_in_year
H0 = 1.4e3 # Grounding line ice thickness
U_inflow = 400/mat.seconds_in_year;   # Inflow velocity
...
U00 = (mat.di*mat.ρ_i*mat.g*H0/(4*mat.B))^mat.n*(Lx/argand_number)
...
#accum[1:3,:].=0.0/mat.seconds_in_year
#accum[end-3:end,:].=0.0/mat.seconds_in_year
...
visc_prms.tau_y = tau_y*0.95#-0.03#*0.95#*0.83
visc_prms.tau_min = 0.1/2/2#0.04
...
#mask[end-40-2:end].=0.0
for i in 0:1000
    if plastic_fail==true 
        if mod(i,2)==0
            local fname = "../data/plume/ex2/rift_margin$Nx"*"_U_inflow_$U_peak"*"slip_frac_$slip_frac"*"yield_$tau_y"*"_"*string(i)*".jld"
            println(fname)
    ...
    for j in 1:200
        # Update 
        #f=max(1-t*Lx/U0/mat.seconds_in_year/5.0,0)
        #if (t*Lx/U0/mat.seconds_in_year)>0.0
        #    f=1.0
        #    slip_frac = 0.9
        #    global U_in = 2^(pow-1)*2^(pow+1)*U_inflow*(1-slip_frac).*((y.*(1.0 .- y))).^pow .+ slip_frac*U_inflow
        #else 
        #    f=1.0
        #end
        global f=ones(Nx+2)
        #global f[end-25:Nx].=0.0
        local dt=ice.update(vel,T,h,U_in/U0,V_inflow/U0,H0/H0,accum*Lx/U0/H0,grid,dim,visc_prms;num_steps=num_steps,num_it=num_it,type="superbee",tau_margin=tau_margin*f,mask=mask)
    ...
    =#
    a = (T.xx .+ T.yy)
    global T1 = a .+ 0.5*sqrt.(a.^2 +T.xy.^2)
    global T2 = a .- 0.5*sqrt.(a.^2 +T.xy.^2)
